...
def _kv_storage_size(cache_obj):
    """Token capacity of a KV storage object (raw tensor or ChunkedKVCache)."""
    if hasattr(cache_obj, "kv_cache_size"):
        return cache_obj.kv_cache_size
    return cache_obj.shape[1]


def _kv_write(cache_obj, start, end, data):
    """Write a [B, num_tokens, H, D] slice into a KV cache."""
    if hasattr(cache_obj, "write"):
        cache_obj.write(start, end, data)
    else:
        cache_obj[:, start:end] = data


def _kv_read(cache_obj, start, end):
    """Read a [B, num_tokens, H, D] slice from a KV cache."""
    if hasattr(cache_obj, "read"):
        return cache_obj.read(start, end)
    return cache_obj[:, start:end]


def _kv_windowed_gather(kv_cache, roped_key, v_new, current_start, current_end,
                        local_attn_size, sink_size, frame_seqlen):
    """Sliding-window KV update for a QVG ``ChunkedKVCache`` (BSHD layout).

    Unlike the raw-tensor path (which physically *rolls* the window and would
    re-materialize quantized spans to BF16), this keeps **absolute** token
    indices and never moves data:

      1. Write the new (already-roped) K/V block at ``[current_start,
         current_end)``.
      2. Free the region that has slid out of the window; a quantized span
         keeps its centroids until *all* of its frames leave the window
         (``ChunkedKVCache.evict_range``).
      3. Return the K/V to attend over = sink frames ``[0, sink_tokens)`` plus
         the most-recent ``(local_attn_size - sink_size)`` frames.

    The attended key *set* (and each key's baked absolute RoPE) is identical to
    the raw-tensor roll, so BF16 output matches the reference bit-for-bit; only
    the storage layout differs.
    """
    kc, vc = kv_cache["k"], kv_cache["v"]
    _kv_write(kc, current_start, current_end, roped_key)
    _kv_write(vc, current_start, current_end, v_new)

    sink_tokens = sink_size * frame_seqlen
    recent_tokens = (local_attn_size - sink_size) * frame_seqlen
    win_start = max(sink_tokens, current_end - recent_tokens)

    if win_start > sink_tokens:
        kc.evict_range(sink_tokens, win_start)
        vc.evict_range(sink_tokens, win_start)

    # Gather live ranges [0, sink_tokens) + [win_start, current_end) (seq dim=1).
    if win_start <= sink_tokens:
        k_cache = kc.read(0, current_end)
        v_cache = vc.read(0, current_end)
    elif sink_tokens == 0:
        k_cache = kc.read(win_start, current_end)
        v_cache = vc.read(win_start, current_end)
    else:
        k_cache = torch.cat([kc.read(0, sink_tokens), kc.read(win_start, current_end)], dim=1)
        v_cache = torch.cat([vc.read(0, sink_tokens), vc.read(win_start, current_end)], dim=1)
...
class CausalWanSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if self.local_attn_size != -1 and hasattr(kv_cache["k"], "read"):
            ...
            # indexing, no physical roll, quantization-aware eviction.
            k_cache, v_cache = _kv_windowed_gather(
                kv_cache, roped_key, v, current_start, current_end,
                self.local_attn_size, self.sink_size, frame_seqlen)
        else:
            if self.local_attn_size == -1:
                # Global cache (no eviction). Indices advance with current_start/end.
                local_end_index = current_end
                local_start_index = current_start
                _kv_write(kv_cache["k"], local_start_index, local_end_index, roped_key)
