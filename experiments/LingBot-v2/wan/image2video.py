...
class WanI2VCausal:
    ...
    def _generate_causal_fast(self,
        ...
        if cache_key in self._t5_cache:
            ...
            if not self.t5_cpu:
                ...
                # Always offload T5 to CPU after the one-time prompt encode: it is
                # unused during the generation loop and VAE decode, so keeping it
                # resident just inflates the end-to-end footprint.
        ...
        # Attention window (== max_attention_size). Sliding-window when
        # local_attn_size>-1, else the whole clip (global cache).
        if self.local_attn_size > -1:
            kv_size = frame_seqlen * self.local_attn_size
        else:
            kv_size = frame_seqlen * lat_f
        # A ChunkedKVCache is absolute-indexed, so its capacity must span every
        # frame slot even under eviction (freed slots cost no GPU memory); the
        # raw-tensor cache stays window-sized.
        cache_tokens = frame_seqlen * lat_f if self.use_chunked_kv else kv_size
        head_dim = model_args.dim // model_args.num_heads
        local_num_heads = model_args.num_heads // self.sp_size
        self_kv_shape = [batch_size, cache_tokens, local_num_heads, head_dim]
        ...
        with (
            ...
            for chunk_id in tqdm(range(num_inference_chunk)):
                ...
                # Every `quant_factor` chunks, compress the most-recent
                # `quant_factor` finalized chunks in-place (decompressed on read).
                # The footprint is printed RIGHT AFTER compression (mirrors
                # HY-WorldPlay) — at that instant the current chunk's KV is not
                # yet written, so the cache is 100% quantized with no bf16 tail;
                # the last such print is the reported steady-state KV footprint.
                # bf16 prints at the same cadence for an apples-to-apples baseline.
                if self.use_chunked_kv:
                    qf = getattr(self.quant_config, "quant_factor", 8) if self.quant_config else 8
                    quant_on = (self.quant_config is not None
                                and getattr(self.quant_config, "quant_type", "none") != "none")
                    # Recency guard: hold back the newest `keep_recent` completed
                    # chunks so the chunk being denoised always attends to clean
                    # BF16 history for its immediate predecessors. Shifting BOTH
                    # ends of the window keeps the spans contiguous, so every
                    # chunk is still quantized exactly once (just later).
                    keep_recent = (getattr(self.quant_config, "quant_keep_recent_chunks", 0)
                                   if self.quant_config else 0)
                    if chunk_id >= qf + keep_recent and (chunk_id - keep_recent) % qf == 0:
                        if quant_on:
                            tokens_per_chunk = chunk_size * frame_seqlen
                            tokens_start = (chunk_id - qf - keep_recent) * tokens_per_chunk
                            tokens_end = (chunk_id - keep_recent) * tokens_per_chunk
                            max_tokens = self_kv_cache[0]["local_end_index"].item()
                            if self.local_attn_size > -1:
                                # Sliding-window eviction: build the span list to
                                # quantize, clamped to what is still resident.
                                #
                                # The sink IS quantized, but as its OWN compact span
                                # [0, sink_tokens): the sink is never evicted, so if we
                                # folded it into a wider [0, tokens_end) span that span
                                # could never be dropped and would pin ~quant_factor
                                # chunks of already-out-of-window "dead" data forever,
                                # inflating the resident KV. Splitting at the sink
                                # boundary keeps the sink codebook tight and lets the
                                # recent spans evict cleanly.
                                #
                                # Later triggers touch only the recent window; clamp
                                # their start up to the recent-window start so we never
                                # read evicted tokens (safe because
                                # quant_factor*chunk_size <= recent window, so a recent
                                # span never straddles the evicted gap).
                                ...
                                sink_keep = min(
                                    ...
                                    sink_tokens)
                                recent_start = tokens_end - (self.local_attn_size - self.sink_size) * frame_seqlen
                                spans = []
                                if tokens_start < sink_tokens:
                                    # Sink part = window ∩ sink region. With a large
                                    # quant_factor this is exactly [0, sink_tokens) —
                                    # the single compact sink span as before — while
                                    # small factors (per-chunk quantization) split the
                                    # sink into per-chunk spans instead of re-quantizing
                                    # the whole sink at every trigger.
                                    spans.append((max(tokens_start, sink_keep),
                                                  min(sink_tokens, tokens_end)))
                                    spans.append((sink_tokens, tokens_end))  # recent
                                else:
                                    spans.append((max(tokens_start, recent_start), tokens_end))
                            else:
                                spans = [(tokens_start, tokens_end)]
                            for lo, hi in spans:
                                if lo < hi:
                                    self._quantize_kv_cache(self_kv_cache, lo, hi, max_tokens)
                            torch.cuda.empty_cache()
                        else:
    ...
    def _quantize_kv_cache(self, self_kv_cache, tokens_to_quantize_start, tokens_to_quantize_end, max_tokens):
        """Compress KV-cache span ``[start, end)`` in-place with QVG k-means PQ.

        Requires ChunkedKVCache storage; no-op when quant_type=='none'. The
        compressed span is decompressed on the fly at attention read time.
        """
        ...
        assert tokens_to_quantize_start <= max_tokens and tokens_to_quantize_end <= max_tokens, (
            f"Tried to quantize [{tokens_to_quantize_start},{tokens_to_quantize_end}) but cache "
            f"only has {max_tokens} written tokens"
        )
        if self.rank == 0:
            cprint(f"Quantizing {tokens_to_quantize_end - tokens_to_quantize_start} tokens "
                   f"[{tokens_to_quantize_start}, {tokens_to_quantize_end}) of {max_tokens}", "light_cyan")
        ...
        # Report the KV footprint right after compressing this span (HY-WorldPlay
        # convention): current chunk not yet written => cache is fully quantized.
        self._print_kv_cache_memory(self_kv_cache)

    def _print_kv_cache_memory(self, self_kv_cache):
        """Report the self-attn KV cache's GPU footprint (per-layer + total MB).

        Offloads every layer to CPU, then onloads layer-by-layer, attributing
        each ``memory_allocated`` increment to that layer. Median increment times
        layer count estimates total KV-cache memory. Mirrors HY-WorldPlay /
        LingBot(v1); works for both ChunkedKVCache and flat-tensor caches.
        """
        if self.rank != 0:
            return
        from termcolor import cprint
        ...
        per_layer = float(np.median(diff_per_layer))
        total = per_layer * len(self_kv_cache)
        cprint(f"[KV-CACHE] Per Layer: {per_layer:.2f} MB | Total KV Cache: {total:.2f} MB "
               f"| Other Resident: {memory_usage[0]:.2f} MB | Peak: {peak:.2f} MB", "light_blue")
    ...
    def _initialize_self_kv_cache(self, num_layers, shape, dtype, device, frame_seqlen=None):
        ...
        When ``self.use_chunked_kv`` is True, allocates QVG ChunkedKVCache (each
        chunk is one latent frame; BF16 chunks are lazily allocated on first
        write, quantized spans stored compressed). Otherwise allocates a flat
        zeroed tensor (original behavior).
        """
        batch_size, kv_size, local_num_heads, head_dim = shape
        if getattr(self, "use_chunked_kv", False):
